...
use crate::useless_conversion::adjustments;
...
fn adjust_to_clone_like(
    ...
    if parent_def.is_diag_item(cx, sym::Clone) {
        let recv_ty = cx.typeck_results().expr_ty(recv);
        return Some(if recv_ty.is_ref() {
            String::new()
        } else {
            String::from("&")
        });
    ...
    if !is_clone_like(cx, path.ident.name, parent_def) {
        return None;
    }
    ...
    // 2. PathBuf -> Path -> &Path
    if ret_ty != *recv_ty_inner {
        if let Some(name) = recv_ty_inner.opt_diag_name(cx)
            && matches!(name, sym::OsStr | sym::Path)
            && let [rest @ .., adjust, _] = cx.typeck_results().expr_adjustments(recv)
            && matches!(
                adjust.kind,
        ...
        {
            let mut prefix = adjustments(cx, recv);
            // Remove the final adjustments
            prefix.pop();
            return Some(prefix);
        }
    ...
    let prefix = adjustments(cx, recv);
    Some(prefix)
}
